    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          further_point1 =  getXY(car_s + look_ahead_distance, global_car_d, map_waypoints_s, 
                                  map_waypoints_x, 
                                  map_waypoints_y);

          further_point2 =  getXY(car_s + look_ahead_distance + 30, global_car_d, map_waypoints_s, 
                                  map_waypoints_x, 
                                  map_waypoints_y);

          further_point3 =  getXY(car_s + look_ahead_distance + 60, global_car_d, map_waypoints_s, 
                                  map_waypoints_x, 
                                  map_waypoints_y);
          ...
          {
            if((double(sensor_fusion[j][5]) <= (car_s + look_ahead_distance)) && (double(sensor_fusion[j][5]) > car_s) && (abs(double(sensor_fusion[j][6]) - global_car_d) < 2))
            {
		 ...
		 //Basic State Machine For Lane Change
          if(change_lanes)
          {
            if(global_car_d < 4)
            {
              ...
              for(int j = 0; j < sensor_fusion.size(); j++)
              {
                  if((double(sensor_fusion[j][5]) <= (car_s + look_ahead_distance)) && (double(sensor_fusion[j][5]) >= (car_s - 5)) && ((double(sensor_fusion[j][6]) - global_car_d) > 2) && ((double(sensor_fusion[j][6]) - global_car_d) < 6))
                  {       
                    /*Can't overtake*/
                    change_lanes = false;
                    break;
              ...
              if(change_lanes)
              {
                ...
                further_point1 =  getXY(car_s + look_ahead_distance, 2, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);

                further_point2 =  getXY(car_s + look_ahead_distance + 30, 4, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);

                further_point3 =  getXY(car_s + look_ahead_distance + 60, 6, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);
          ...
          std::cout <<  "Left to Centre" << std::endl;
            ...
            else if(global_car_d < 8)
            {
              ...
              for(int j = 0; j < sensor_fusion.size(); j++)
              {
                  if((double(sensor_fusion[j][5]) <= (car_s + look_ahead_distance)) && (double(sensor_fusion[j][5]) >= (car_s - 5)) && ((double(sensor_fusion[j][6]) - global_car_d) > 2))
                  {       
                    /*Can't overtake*/
                    change_lanes = false;
                    lane_change = 's';
              ...
              if(lane_change == 'l')
              {
                ...
                further_point1 =  getXY(car_s + look_ahead_distance, 6, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);
                ...
                further_point3 =  getXY(car_s + look_ahead_distance + 60, 2, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);
          ...
          std::cout <<  "Centre to Left" << std::endl;
              ...
              else if(lane_change == 'r')
              {
                ...
                further_point2 =  getXY(car_s + look_ahead_distance + 30, 8, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);

                further_point3 =  getXY(car_s + look_ahead_distance + 60, 10, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);
          ...
          std::cout <<  "Centre to Right" << std::endl;
            ...
            else
            {
              ...
              {
                /*Furthest spline points progressing towards neighboring left lane.*/
                further_point1 =  getXY(car_s + look_ahead_distance, 8, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);

                further_point2 =  getXY(car_s + look_ahead_distance + 30, 6, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);

                further_point3 =  getXY(car_s + look_ahead_distance + 60, 4, map_waypoints_s, 
                                        map_waypoints_x, 
                                        map_waypoints_y);
